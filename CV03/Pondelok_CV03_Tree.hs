module Pondelok_CV03_Tree where
...
e' :: BVS Int
e' = Node (Node Nil 2 Nil) 5 (Node Nil 7 Nil)
...
insert x t@(Node left value right) | x == value  = t 
                                   | x < value  = Node (insert x left) value right
                                   | otherwise  = let right' = insert x right in Node left value right'
